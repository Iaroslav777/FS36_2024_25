...
import {
  ...
  fetchPostById,
  fetchCommentById,
} from "../services/api"; // API для деталей (прямі запити для деталей)
    ...
    <>
      <div className="w-full max-w-6xl bg-white shadow-2xl rounded-xl overflow-hidden flex flex-col min-h-[calc(100vh-2rem)]">
        ...
        <footer className="p-4 bg-gray-800 text-white text-center text-sm rounded-b-xl shadow-inner">
          <p>© 2025 Домашнє Завдання 19. Бабік Ярослав.</p>
        </footer>
      </div>
    </>
  );
    ...
    <>
      <div className="text-center p-8 bg-white rounded-lg shadow-md border border-gray-200">
        <h2 className="text-3xl font-bold text-gray-800 mb-4">
          Вітаємо на сторінці Домашнього Завдання 19!
        </h2>
...
function UsersPage() {
  ...
  useEffect(() => {
    ...
    // Перевіряємо !loading, щоб уникнути повторних запитів під час активного завантаження
    if (users.length === 0 && !loading) {
      dispatch(fetchUsers());
    }
  }, [dispatch, users.length, loading]); // Додаємо loading до залежностей
...
function CommentsPage() {
  ...
  }, [dispatch, comments.length, loading]);
